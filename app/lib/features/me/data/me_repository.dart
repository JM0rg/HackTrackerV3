...
class MeRepository {
  ...
  Future<Person> ensureMe() async {
    ...
    final me = await (_db.select(_db.people)..where((t) => t.id.equals(id))).getSingle();
  ...
  Future<void> updateMe({
    ...
    final now = _now();
    final display = lastName.trim().isEmpty ? firstName.trim() : '${firstName.trim()} ${lastName.trim()}';
    ...
    if (personal != null) {
      await (_db.update(_db.players)..where((t) => t.id.equals(personal.id))).write(
            PlayersCompanion(
              firstName: Value(firstName.trim().isEmpty ? 'Me' : firstName.trim()),
              lastName: Value(lastName.trim()),
              updatedAt: Value(now),
  ...
  Future<void> attachMeToNewTeam(String teamId) async {
    final me = await ensureMe();
    final already = await (_db.select(_db.players)
          ..where(
            (t) =>
  ...
  Future<void> setRosterSlotAsMe({
    ...
    await _db.transaction(() async {
      final current = await (_db.select(_db.players)
            ..where(
              (t) =>
                  t.teamId.equals(teamId) &
                  t.personId.equals(me.id) &
      ...
      for (final row in current) {
        ...
        await (_db.update(_db.players)..where((t) => t.id.equals(row.id))).write(
              PlayersCompanion(
  ...
  Future<PersonalTeam?> createPersonalTeam(String name) async {
    ...
    final now = _now();
    final id = _uuid.v4();
    await _db.into(_db.personalTeams).insert(
    ...
    return (_db.select(_db.personalTeams)..where((t) => t.id.equals(id)))
        .getSingle();
  }
  ...
  Stream<List<Game>> watchMyGames() {
    return watchMe().asyncExpand((me) {
      ...
      final query = _db.select(_db.games).join([
        innerJoin(
          _db.lineupSlots,
  ...
  Stream<List<Team>> watchTeamsIPlayOn() {
    return watchMe().asyncExpand((me) {
      ...
      final query = _db.select(_db.teams).join([
        innerJoin(
          _db.players,
          _db.players.teamId.equalsExp(_db.teams.id),
        ),
      ])
        ..where(
          _db.players.personId.equals(me.id) &
  ...
  Stream<List<YouPaRow>> watchMyPaRows() {
    return watchMe().asyncExpand((me) {
      ...
      final query = _db.select(_db.plateAppearances).join([
        innerJoin(
          _db.games,
        ...
        return [
          for (final row in rows)
            YouPaRow(
              ...
              result: PaResult.fromWire(row.readTable(_db.plateAppearances).result),
