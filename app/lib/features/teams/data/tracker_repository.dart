...
class TrackerRepository {
  ...
  Stream<List<Player>> watchPlayers(String teamId) {
    return (_db.select(
      _db.players,
    )..where((t) => t.teamId.equals(teamId) & t.deletedAt.isNull())).watch();
  }
  ...
  Stream<List<Player>> watchPlayersForGame(String gameId) {
    return watchLineup(gameId).asyncMap((slots) async {
      ...
      return (_db.select(
        _db.players,
      )..where((t) => t.id.isIn(slots.map((s) => s.playerId)))).get();
    });
  }
  ...
  Future<void> _seedLineupFromLastGame({
    ...
    final slots =
        await (_db.select(_db.lineupSlots)
              ..where(
                (t) =>
                    t.gameId.equals(previous.first.id) & t.deletedAt.isNull(),
              )
  ...
  /// The newest team game, for the start sheet's defaults.
  Future<Game?> lastTeamGame(String teamId) {
    return (_db.select(_db.games)
          ..where(
            (t) =>
                t.teamId.equals(teamId) &
  ...
  Future<List<String>> gameCompetitionIds(String gameId) async {
    final rows = await (_db.select(
      _db.gameCompetitions,
    )..where((t) => t.gameId.equals(gameId) & t.deletedAt.isNull())).get();
    return [for (final r in rows) r.competitionId];
  }
  ...
  Future<void> setLineup({
    ...
    await (_db.update(
      _db.lineupSlots,
    )..where((t) => t.gameId.equals(gameId))).write(
  ...
  Future<List<InviteCode>> invites(String teamId) {
    return (_db.select(
      _db.inviteCodes,
    )..where((t) => t.teamId.equals(teamId) & t.deletedAt.isNull())).get();
  }
}
