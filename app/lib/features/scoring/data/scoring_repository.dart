...
class ScoringRepository {
  ...
  Future<TeamSettings> teamSettings(String? teamId) async {
    ...
    final team = await (_db.select(_db.teams)..where((t) => t.id.equals(teamId)))
        .getSingleOrNull();
  ...
  Future<void> recordPa({
    ...
    final now = _now();
    await _db.into(_db.plateAppearances).insert(
  ...
  Future<void> setRunsOnPlay({
    ...
    if (result == PaResult.out || result == PaResult.sacFly) {
      ...
      await _writePa(paId, PlateAppearancesCompanion(runsOnPlay: Value(runs)));
    }
    await rebuild(game.id);
  }
  ...
  Future<void> setOutKind({
    ...
    if (kind == null) {
      ...
      await _writePa(
        paId,
        _resolveOut(kind: kind, runScored: runScored, runsOnPlay: row.runsOnPlay),
      );
    }
    await rebuild(game.id);
  }
  ...
  Future<void> setBatterScored({
    ...
    await _writePa(paId, PlateAppearancesCompanion(batterScored: Value(scored)));
    await rebuild(game.id);
  }
  ...
  Future<void> undoLast(Game game) async {
    ...
    final now = _now();
    final newestIsEvent = lastEvent != null &&
        (lastPa == null || lastEvent.sequence > lastPa.sequence);

    if (newestIsEvent) {
      await (_db.update(_db.gameEvents)..where((t) => t.id.equals(lastEvent.id)))
          .write(
        GameEventsCompanion(
          deletedAt: Value(now),
          updatedAt: Value(now),
      ...
      await _writePa(lastPa.id, PlateAppearancesCompanion(deletedAt: Value(now)));
    }
    await rebuild(live.id);
  }

  Future<void> bumpTheirHalfRuns({required Game game, required int delta}) async {
    final live = await this.game(game.id) ?? game;
  ...
  /// A personal game keeping score: a teammate crossed in our half. Your own
  /// RBI come from your plate appearances, so this is only for other people.
    ...
    return [
      for (final pa in pas)
        PaEventInput(
          ...
          batterIsMale: genderByPlayerId[pa.playerId] == PlayerGender.male,
  ...
  Future<ReplayedGame> replay(String gameId) async {
    ...
    final settings = await teamSettings(game.teamId);
  ...
  Future<void> rebuild(String gameId) async {
    ...
    for (final played in state.pas) {
      ...
      final unchanged = row.rbi == played.rbi &&
    ...
    await (_db.update(_db.games)..where((t) => t.id.equals(gameId))).write(
      GamesCompanion(
        ...
        status: Value(state.pas.isEmpty ? game.status : 'live'),
        updatedAt: Value(now),
        syncState: const Value(1),
      ),
    );
  ...
  Future<void> _writeInnings(Game game, ReplayedGame state) async {
    final existing = await (_db.select(_db.gameInnings)
    ...
    for (final line in state.innings) {
      ...
      if (row == null) {
        await _db.into(_db.gameInnings).insert(
      ...
      final unchanged = row.ourRuns == line.ourRuns &&
          row.theirRuns == line.theirRuns &&
          row.deletedAt == null;
      if (unchanged) continue;
      await (_db.update(_db.gameInnings)..where((t) => t.id.equals(row.id))).write(
  ...
  Future<PlateAppearance?> _pa(String id) {
    return (_db.select(_db.plateAppearances)..where((t) => t.id.equals(id)))
        .getSingleOrNull();
  }
  ...
  Future<Map<String, String>> _genders(List<String> playerIds) async {
    ...
    final rows =
        await (_db.select(_db.players)..where((t) => t.id.isIn(playerIds))).get();
  ...
  Future<int> _nextSequence(String gameId) async {
    final pas = await (_db.select(_db.plateAppearances)
  ...
  Future<void> _writePa(String paId, PlateAppearancesCompanion values) {
    return (_db.update(_db.plateAppearances)..where((t) => t.id.equals(paId))).write(
