...
class ContactFieldGeometry {
  ContactFieldGeometry(this.size);
  final Size size;
  double get radius => math.min(size.width / 1.65, (size.height - 58) / 1.12);
  Offset get home => Offset(size.width / 2, radius + 18);
...
class _ContactFieldState extends State<ContactField> {
  ...
  Widget build(BuildContext context) => LayoutBuilder(
    builder: (context, c) {
      final size = Size(c.maxWidth, c.maxWidth * .68 + 44);
      final g = ContactFieldGeometry(size);
      Offset aim(Offset p) =>
      ...
      return Semantics(
        label:
            'Field, 65 foot basepaths, 300 foot fence. Tap or drag to where the ball was first fielded or landed. Region buttons are also available.',
