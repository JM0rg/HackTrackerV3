...
void main() {
  ...
  test('personal PAs copy personId and stay off the team leaderboard', () async {
    ...
    final lineup = await (db.select(db.players)
    ...
    expect(youPaInputs(youRows, const YouFilter.personal(), person.id).length, 1);
    expect(youPaInputs(youRows, YouFilter.team(team.id), person.id).length, 1);
  ...
  group('personal teams', () {
    test('saving, listing and forgetting', () async {
      ...
      expect(
        (await me.personalTeams()).map((t) => t.name),
        ['Alley Cats', 'Tuesday Crew'],
      );
    ...
    test('a game tagged to a saved team keeps both the tag and the name',
        () async {
      await me.ensureMe();
      final crew = await me.createPersonalTeam('Tuesday Crew');
  ...
  test('undo after a single restores empty bases and the same batter', () async {
    final team = await tracker.createTeam(name: 'Club');
